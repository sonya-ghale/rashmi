...
export const register = catchAsyncErrors(async (req, res, next) => {
    try {
        ...
        const registrationAttempt = await User.find({ email, accountVerified: false });
        if (registrationAttempt.length > 0) {
            return next(new ErrorHandler("You have already registered. Please verify your account.", 400));
        }
...
export const verifyOTP = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.password !== req.body.confirmPassword) {
        return next(new ErrorHandler("Passwords & Confirm Password do not match.", 400));
    }
