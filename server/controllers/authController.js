...
import { generateForgotPasswordEmailTemplate } from "../utils/emailTempletes.js";
...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (registrationAttempt.length > 0) {
      return next(
        new ErrorHandler(
          "You have already registered. Please verify your account.",
          400
        )
      );
    }
    ...
    if (password.length < 6 || password.length > 20) {
      return next(
        new ErrorHandler("Password must be between 6 to 20 characters.", 400)
      );
    }
    ...
    newUser.verificationCodeExpires = new Date(Date.now() + 10 * 60 * 1000); // 10 mins
...
export const verifyOTP = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (!userAllEntries || userAllEntries.length === 0) {
      return next(
        new ErrorHandler("No unverified user found with this email.", 404)
      );
    }
    ...
    if (!user) {
      return next(new ErrorHandler("User entry not found.", 404));
    }
    ...
    return next(
      new ErrorHandler("Internal server error during OTP verification", 500)
    );
  }
});
